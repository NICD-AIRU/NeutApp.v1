...
build_titer_annot <- function(pid_titer, pos_labels) {
  if (is.null(pid_titer) || nrow(pid_titer) == 0)
    return(data.frame(series = character(0), sample_id = character(0),
                       metric = character(0), x = numeric(0), y = numeric(0),
                       label = character(0)))
  mk_series <- function(df) paste0(pos_labels[df$sample_id], ": ", df$sample_id, " / ", df$virus_id)
  ic50_annot <- pid_titer %>% filter(!is.na(titer)) %>%
    { data.frame(series = mk_series(.), sample_id = .$sample_id, metric = "IC50",
                 x = .$titer, y = 50 - 7,
                 label = paste0("IC50=", vapply(.$titer, fmt_titer_label, character(1))),
                 stringsAsFactors = FALSE) }
  ic80_annot <- pid_titer %>% filter(!is.na(titer_ic80)) %>%
    { data.frame(series = mk_series(.), sample_id = .$sample_id, metric = "IC80",
                 x = .$titer_ic80, y = 80 + 7,
