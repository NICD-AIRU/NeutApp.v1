...
server <- function(input, output, session) {
  ...
  observeEvent(input$plate_files, {
    ...
    for (i in seq_len(nrow(finfo))) {
      ...
      # Convert legacy .xls to .xlsx before reading
      if (ext == "xls") {
        xlsx_path <- paste0(fpath, ".xlsx")
        converted <- tryCatch({
          # Use writexl/readxl round-trip: read all sheets and re-write as xlsx
          sheet_names <- readxl::excel_sheets(fpath)
          wb_conv <- openxlsx::createWorkbook()
          for (sn in sheet_names) {
            sdata <- readxl::read_excel(fpath, sheet = sn, col_names = FALSE, col_types = "text")
            openxlsx::addWorksheet(wb_conv, sn)
            openxlsx::writeData(wb_conv, sn, sdata, colNames = FALSE)
          }
          openxlsx::saveWorkbook(wb_conv, xlsx_path, overwrite = TRUE)
          TRUE
        }, error = function(e) {
          message("[WARN] XLS conversion failed for ", fname, ": ", e$message)
          FALSE
        })
        if (converted) {
          fpath <- xlsx_path
          showNotification(paste0("Converted ", fname, " from .xls to .xlsx"),
  ...
  output$dl_xlsx <- downloadHandler(
    ...
    content = function(file) {
      ...
      # Display as %, highlight values 10%-30% with orange and > 30% with red font
      if (nrow(ctrl_summary_df) > 0) {
        ...
        # Orange: between 10% and 30%
        conditionalFormatting(
          wb, "05_control_summary",
          cols  = 7,
          rows  = pct_rows_05,
          rule  = "AND(G2>=0.1,G2<=0.3)",
