...
# target: the IC threshold being classified against (50 for IC50, 80 for IC80)
# Returns: "FLAT" | "Titered" | "Not-Titered"
# NOTE: outcome is threshold-specific -- a curve can be "Titered" for IC80
# (crosses 80%) while being "Not-Titered" for IC50 (never dips below 50%,
# i.e. min inhibition > 50), or vice versa. Each metric must be classified
# against its own threshold rather than reusing a single IC50-based outcome.
determine_outcome <- function(inhibitions_pct, target = 50) {
  ...
  if (mx < target) return("FLAT")
  if (mn > target) return("Not-Titered")
  "Titered"
}
...
process_plate_full <- function(raw_mat, pid, helper_table,
  ...
  for (k in seq_len(nrow(bnab_exps))) {
    ...
    outcome_val <- determine_outcome(inh_pct, 50)  # IC50-based outcome (unchanged semantics)
    outcome80   <- determine_outcome(inh_pct, 80)  # IC80-specific outcome
    ...
    # 4PL fit -- attempted whenever the curve isn't FLAT (max inhibition >= 50%).
    # This covers "Not-Titered" curves that never dip below 50% but still cross
    # 80% (e.g. a very potent sample), so IC80 still gets fit/reported even
    # though IC50 cannot be estimated from this data.
    fit4 <- if (outcome_val != "FLAT") {
    ...
    # Mask 4PL estimates that aren't actually anchored by a threshold crossing --
    # widening the fit above (to Not-Titered-by-IC50 curves) lets drc estimate an
    # IC50 by extrapolation even when the data never crosses 50%; that estimate
    # isn't meaningful and must not be reported. Each metric is only kept when its
    # OWN threshold was actually crossed ("Titered" for that metric).
    if (!identical(outcome_val, "Titered")) fit4$ic50_4pl <- NA_real_
    if (!identical(outcome80,   "Titered")) fit4$ic80_4pl <- NA_real_
    ...
    titer_rows[[k]] <- data.frame(
      ...
      outcome             = outcome_val,   # IC50-based outcome (Titered / FLAT / Not-Titered)
      outcome_ic80        = outcome80,     # IC80-specific outcome -- use this to gate IC80 display
      titer               = best_ic50,    # best available IC50 -- used by plots
      titer_ic80          = best_ic80,    # best available IC80 -- used by plots
      titer_id            = 50L,
      stringsAsFactors = FALSE
    )
  }
...
# Format a titer value for on-plot / label display. Uses scientific notation with a
# CAPITAL "E" (e.g. "4.00E-10") for very small values, otherwise a plain rounded number.
fmt_titer_label <- function(x, digits = 3) {
  if (is.na(x) || !is.finite(x)) return("NA")
  s <- if (abs(x) > 0 && abs(x) < 0.001) {
...
# Build long-format annotation data for on-plot IC50/IC80 labels.
# One row per sample per metric that was actually reached (Titered), so a curve
# that crosses 80% but never dips below 50% still gets its IC80 label even though
# it has no IC50 label (and vice versa).
build_titer_annot <- function(pid_titer, pos_labels) {
  empty_annot <- data.frame(series = character(0), sample_id = character(0),
                             metric = character(0), x = numeric(0), y = numeric(0),
                             label = character(0), stringsAsFactors = FALSE)
  if (is.null(pid_titer) || nrow(pid_titer) == 0) return(empty_annot)

  # dplyr::transmute (not a raw data.frame() call) correctly recycles scalar
  # columns (e.g. metric = "IC50", y = 43) down to 0 rows when the filtered
  # input is empty -- a plain data.frame() call errors in that case because it
  # can't reconcile 0-length columns with length-1 scalar columns.
  ic50_annot <- pid_titer %>% filter(!is.na(titer)) %>%
    transmute(series    = paste0(pos_labels[sample_id], ": ", sample_id, " / ", virus_id),
              sample_id = sample_id,
              metric    = "IC50",
              x         = titer,
              y         = 50 - 7,
              label     = paste0("IC50=", vapply(titer, fmt_titer_label, character(1))))
  ic80_annot <- pid_titer %>% filter(!is.na(titer_ic80)) %>%
    transmute(series    = paste0(pos_labels[sample_id], ": ", sample_id, " / ", virus_id),
              sample_id = sample_id,
              metric    = "IC80",
              x         = titer_ic80,
              y         = 80 + 7,
              label     = paste0("IC80=", vapply(titer_ic80, fmt_titer_label, character(1))))
  bind_rows(ic50_annot, ic80_annot)
}
...
build_neut_ggplot <- function(plot_data, titer_annot, colours, shape_vals,
  ...
  # On-plot IC50 / IC80 value labels, coloured to match each sample's series.
  # Each metric is placed independently (IC50 just below the 50% line, IC80 just
  # above the 80% line) so a curve reaching only one threshold still shows that value.
  if (!is.null(titer_annot) && nrow(titer_annot) > 0) {
    p <- p + geom_label(data = titer_annot,
                         aes(x = x, y = y, label = label, colour = series),
                         inherit.aes = FALSE, show.legend = FALSE,
                         size = 2.5, fontface = "bold", label.size = 0,
...
make_neut_plot_gg <- function(pid_inh, pid_titer, titer_id, pid_layout) {
  ...
  titer_annot <- build_titer_annot(pid_titer, pos_labels)
  ...
  dashboardBody(
    tabItems(
      ...
      tabItem("titers",
        ...
        fluidRow(
          box(title = "Wide Titer Heatmap -- Sample x Virus", width = 12,
            ...
            fluidRow(
              ...
              column(8,
                p(style = "font-size:12px; color:#555; margin-top:26px;",
                  "Rows = Sample | Columns = Virus | ",
                  tags$b("Geomean"), " = geometric mean across viruses (per sample).")
              )
            ),
          ...
          box(title = "Fields Legend -- 07_titers", width = 6, status = "info",
            tags$table(class = "table table-condensed", style = "font-size:12px;",
              ...
              tags$tbody(
                ...
                tags$tr(tags$td("outcome"),         tags$td("IC50 outcome: Titered = 50% threshold crossed; FLAT = max inhibition < 50%; Not-Titered = min inhibition already > 50%")),
                tags$tr(tags$td("outcome_ic80"),    tags$td("IC80 outcome (classified independently of IC50): Titered = 80% threshold crossed; FLAT = max inhibition < 80%; Not-Titered = min inhibition already > 80%")),
...
server <- function(input, output, session) {
    ...
    combined <- titer_combined_long() %>%
      mutate(
        IC50_pb  = dplyr::case_when(
          ...
          outcome == "Not-Titered" ~ "NT",
          !is.na(ic50_pb)          ~ as.character(round(ic50_pb, 2)),
          TRUE                     ~ "NA"),
        IC80_pb  = dplyr::case_when(
          outcome_ic80 == "FLAT"        ~ as.character(round(start_concentration, 2)),
          outcome_ic80 == "Not-Titered" ~ "NT",
        ...
        IC80_4PL = dplyr::case_when(
          ...
          TRUE                            ~ "NA"),
        # Uppercase any scientific notation (e.g. "4e-10" -> "4E-10")
        IC50_pb   = toupper(IC50_pb),
        IC80_pb   = toupper(IC80_pb),
        IC50_4PL  = toupper(IC50_4PL),
        IC80_4PL  = toupper(IC80_4PL),
  ...
  output$dt_wide_titers <- renderDT({
    ...
    raw_titers <- collect_results("titers") %>%
      ...
      summarise(
        metric_val = {
          # Use the metric-specific outcome (IC50 vs IC80) so IC80 values still
          # show for curves that cross 80% but never dip below 50% (and vice versa).
          oc <- if (input$wide_titer_metric == "IC50") outcome[1] else outcome_ic80[1]
        ...
        disp_label = {
          oc <- if (input$wide_titer_metric == "IC50") outcome[1] else outcome_ic80[1]
          sc <- start_concentration[1]
          if      (oc == "FLAT")         as.character(round(sc, 2))
          else if (oc == "Not-Titered")  "NT"
      ...
      mutate(disp_label = ifelse(
        ...
        disp_label),
        # Uppercase any scientific notation (e.g. "4e-10" -> "4E-10")
        disp_label = toupper(disp_label))

    # Numeric wide table -- for Geomean and colour scale only
    # Rows = Sample (bnAb / sample_id); Columns = Virus
    wide_num <- raw_titers %>%
      dplyr::select(sample_id, virus_id, metric_val) %>%
      pivot_wider(names_from = virus_id, values_from = metric_val) %>%
      arrange(sample_id)

    # Label wide table -- for cell display text
    wide_lbl <- raw_titers %>%
      dplyr::select(sample_id, virus_id, disp_label) %>%
      pivot_wider(names_from = virus_id, values_from = disp_label) %>%
      arrange(sample_id) %>%
      # Any NA from pivot_wider (missing sample/virus combos) -> "NA"
      mutate(across(-sample_id, ~ ifelse(is.na(.), "NA", .)))

    virus_cols   <- setdiff(colnames(wide_num), "sample_id")
    all_num_cols <- c(virus_cols, "Geomean")

    wide_num <- wide_num %>% rowwise() %>%
      mutate(Geomean = {
        vals <- c_across(all_of(virus_cols))
    ...
    wide_disp <- wide_lbl %>% rename(Sample = sample_id)
    wide_disp[["Geomean"]] <- toupper(ifelse(
  ...
  output$dl_xlsx <- downloadHandler(
    ...
    content = function(file) {
      ...
      if (nrow(all_inh) > 0) {
        ...
        # Alternating grey/white row banding, one band per dilution series
        # (each contiguous block of rows sharing the same plate/sample/virus).
        grp_key <- paste(all_inh_xl$plate_id, all_inh_xl$sample_id, all_inh_xl$virus_id, sep = "||")
        grp_id  <- cumsum(c(TRUE, grp_key[-1] != grp_key[-length(grp_key)]))
        rle_grp <- rle(grp_id)
        grp_end   <- cumsum(rle_grp$lengths)
        grp_start <- grp_end - rle_grp$lengths + 1
        band_style <- createStyle(fgFill = "#EDEDED")
        n_cols_06  <- ncol(all_inh_xl)
        for (gi in seq_along(rle_grp$lengths)) {
          if (gi %% 2 == 0) {
            addStyle(wb, "06_percent_inhibition", band_style,
                     rows = (grp_start[gi] + 1):(grp_end[gi] + 1),  # +1: header occupies row 1
                     cols = 1:n_cols_06, gridExpand = TRUE, stack = TRUE)
          }
        }
      }
      ...
      # Result is uppercased so any scientific notation reads "4E-10" not "4e-10".
      fmt_ic <- function(value, outcome, start_conc) {
        toupper(dplyr::case_when(
      ...
      for (pid in names(rv$results)) {
        ...
        for (si in seq_len(nrow(tdf))) {
          ...
          titer_rows_xl[[length(titer_rows_xl)+1]] <- data.frame(
            ...
            ic80_pointbased = fmt_ic(row_t$ic80_pb,   oc80, sc),
            # 4PL model results
            ...
            ic80_4pl        = fmt_ic(row_t$ic80_4pl,  oc80, sc),
      ...
      # IC50 and IC80 are gated by their OWN threshold-specific outcome (outcome /
      # outcome_ic80) so a curve that crosses 80% but never dips below 50% (or
      # vice versa) still reports the metric it did reach.
      titer_src_xl <- collect_results("titers") %>%
        group_by(virus_id, sample_id) %>%
        filter(plate_id == first(plate_id)) %>%
        summarise(
          outcome_val      = outcome[1],
          outcome_ic80_val = outcome_ic80[1],
          ...
          IC80 = {
            oc <- outcome_ic80[1]; v4 <- ic80_4pl[1]; vpb <- ic80_pb[1]
            if (oc %in% c("FLAT","Not-Titered")) NA_real_
            else if (!is.na(v4) && is.finite(v4)) v4
            else if (!is.na(vpb) && is.finite(vpb)) vpb
            else NA_real_
      ...
      # value_col: "IC50" or "IC80"; outcome_col: name of the matching threshold-specific
      # outcome column ("outcome_val" for IC50, "outcome_ic80_val" for IC80).
      # Rows = Sample (bnAb / sample_id); Columns = Virus.
      make_wide_xl <- function(src, value_col, outcome_col) {
        # Numeric pivot -- used only for Geomean calculation
        num_wide <- src %>%
          dplyr::select(sample_id, virus_id, val = all_of(value_col)) %>%
          group_by(sample_id, virus_id) %>%
          summarise(val = { v <- val[!is.na(val)]; if (length(v) == 0) NA_real_ else v[1] },
                    .groups = "drop") %>%
          pivot_wider(names_from = virus_id, values_from = val) %>% arrange(sample_id) %>%
          rowwise() %>%
          mutate(Geomean = {
            vs <- c_across(-sample_id); vs <- vs[!is.na(vs) & is.finite(vs) & vs > 0]
            if (length(vs) == 0) NA_real_ else round(exp(mean(log(vs))), 4)
          }) %>% ungroup() %>% rename(Sample = sample_id)

        # Label pivot: FLAT -> start_conc, Not-Titered -> "NA", Titered -> numeric or "NA"
        lbl_wide <- src %>%
          mutate(disp = dplyr::case_when(
            .data[[outcome_col]] == "FLAT"        ~ as.character(round(start_conc, 4)),
            .data[[outcome_col]] == "Not-Titered" ~ "NT",
            !is.na(.data[[value_col]]) & is.finite(.data[[value_col]]) ~
              as.character(round(.data[[value_col]], 4)),
            TRUE ~ "NA")) %>%
          dplyr::select(sample_id, virus_id, disp) %>%
          pivot_wider(names_from = virus_id, values_from = disp) %>% arrange(sample_id) %>%
        ...
        # Uppercase any scientific notation (e.g. "4e-10" -> "4E-10") across all
        # value columns (Sample column is text and unaffected: no "e" pattern in IDs
        # is expected, but round-numeric columns are what this targets).
        mutate(lbl_wide, across(-Sample, toupper))
      }

      for (metric in c("IC50","IC80")) {
        oc_col <- if (metric == "IC50") "outcome_val" else "outcome_ic80_val"
        wide <- make_wide_xl(titer_src_xl, metric, oc_col)
        ws <- paste0(metric, "_wide")
        addWorksheet(wb, ws)
        writeData(wb, ws,
                  data.frame(V1 = paste0(metric, " -- Rows = Sample, Columns = Virus; 4PL preferred; point-based fallback; FLAT = highest conc/lowest dilution assessed; NT = Not-Titered")),
